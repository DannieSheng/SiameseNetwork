...
#class EarlyStopping:
#    """Early stops the training if validation loss doesn't improve after a given patience."""
#    def __init__(self, patience=7, start_epoch=160, verbose=False):
#        """
#        Args:
#            patience (int): How long to wait after last time validation loss improved.
#                            Default: 7
...
#        self.thres   = 0.00001*patience
#        self.y1 = 0
#        self.score = np.Inf
#
#    def __call__(self, epoch, val_loss, model, end_dim):
##        pdb.set_trace()
#        if epoch == self.start_epoch + self.patience:
##            try:
#            self.score = slope(0, self.y1, self.patience, val_loss)
##            except:
##                score = np.Inf
##            if (np.abs(score) < self.thres) or (score>0):
#            if self.score > -self.thres:
#                print(f'EarlyStop!')
#                self.early_stop = True
#            else:
...
#        self.thres   = 0.05
#
#    def __call__(self, val_loss, model, end_dim):
#
#        score = -val_loss
#
...
#            self.counter = 0
#
#    def save_checkpoint(self, val_loss, model, end_dim):
#        '''Saves model when validation loss decrease.'''
#        if self.verbose:
#            print(f'Validation loss decreased ({self.val_loss_min:.6f} --> {val_loss:.6f}).  Saving model ...')
#        if not os.path.exists('./' + str(end_dim)):
