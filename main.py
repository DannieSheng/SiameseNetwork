...
=======
    savepath_data_fold = r'T:\Results\AnalysisDroneData\Siamese\CLMB STND 2019 Flight Data\100081_2019_06_11_17_57_06\usegt\use_all_classes\{}\saved_data\fold{}'.format(paras['end_dim'], idx_fold)
    savepath_fold =  paras['savepath']  + '/fold' + str(idx_fold)
    if not os.path.exists(savepath_fold):
        os.makedirs(savepath_fold)     
>>>>>>> 8f2ad42e70c0083b23330ab611aa57df78a63ee5
...
=======
    count_all += 1
    len_all = []
    if os.path.exists(os.path.join(savepath_data_fold, 'data.pkl')):
        paras = pickle.load(open(os.path.join(savepath_data_fold, 'parameters.pkl'), 'rb'))
        all_data = pickle.load(open(os.path.join(savepath_data_fold, 'data.pkl'), 'rb'))
