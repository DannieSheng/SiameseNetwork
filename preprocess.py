...
parameters['labelpath'] = parameters['hyperpath'].replace('dataPerClass', 'groundTruth')
...
while all(l>=2 for l in len_all): #& count_all<10:
    ...
    for idx, classn in enumerate(paras['name_class']):
        ...
        if idx >0:
            list_file = list(set(list_file)-set(paras['selected_file'])) # if file has been selected before, disgard the file for this time 
